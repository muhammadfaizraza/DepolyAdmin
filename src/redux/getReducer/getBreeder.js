...
export const fetchbreeder = createAsyncThunk('/Breederget/fetch', async({SearchCode,SearchTitle,SearchStartDate,SearchEndStart}) => {
    const res = await axios.get(`${window.env.API_URL}/Breederget?shortCode=${SearchCode}&NameEn=${SearchTitle}&startdate=${SearchStartDate}&endDate=${SearchEndStart}&limit=${'2000'}`);
